...
- `node` 已具备最小运行时：
  ...
  - `sink = ingress + inject`
- `node egress` 已支持 `[[egress.groups]]` 的“组内 failover、组间 fanout”。
- `node sink` 的 `inject` 路径已支持 SNMPv1 Trap `agent-addr` 源 IP 修正。
- `node export` 已支持 TCP 长连接订阅、多个客户端并发消费和慢客户端隔离。

当前实现入口：

- [sender/sender.go](/home/qqy/TrapTunnel/sender/sender.go)
- [receiver/receiver.go](/home/qqy/TrapTunnel/receiver/receiver.go)
- [cmd/node/main.go](/home/qqy/TrapTunnel/cmd/node/main.go)

配置兼容状态：

- `cmd/node -c node.toml` 只读取统一 TOML 配置格式
- 不保留 `sender.conf` / `receiver.conf` 的兼容加载
- 现网切换到 `node` 时需要将旧配置手工改写为 `node.toml`
...
capture_read_buffer_size = 2048
capture_read_backoff_ms = 100
ingress_read_buffer_size = 8192
...
- `export` 是服务端被动接受连接，不负责反向重连客户端。
- 客户端若因“慢客户端被断开”或网络异常收到 EOF / write error，应自行发起重连。
- 当前所有运行时缓冲和超时常量都已收敛到 `[tuning]` 配置段；协议字段值和 BER tag 这类协议常量不属于运行时调优项，不应做成配置。

运行时调优默认值：

- `tuning.pipeline_buffer_size = 1024`
- `tuning.capture_read_buffer_size = 2048`
- `tuning.capture_read_backoff_ms = 100`
- `tuning.ingress_read_buffer_size = 8192`
- `tuning.max_frame_total_length = 10485760`
- `tuning.egress_group_buffer_size = 1024`
- `tuning.egress_dial_timeout_ms = 5000`
- `tuning.egress_write_timeout_ms = 5000`
- `tuning.egress_backoff_max_ms = 30000`
- `tuning.egress_backoff_jitter_pct = 20`
- `tuning.export_client_buffer_size = 1024`
- `tuning.export_write_timeout_ms = 5000`

`export.slow_client_policy` 可选值：
...
- `disconnect` 适合优先保护主链路
- `drop_oldest` 适合优先保留最新事件
- `drop_newest` 适合优先保留已排队事件
...
- [旧配置手工迁移指引](/home/qqy/TrapTunnel/docs/manual-config-migration.md)
